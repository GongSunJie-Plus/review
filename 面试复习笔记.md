...
设置自动填充剩余空间的元素 width: 100%，此时大小为父元素width大小400px，由于flex布局的存在
...
https://blog.csdn.net/weixin_30851867/article/details/94955591

- Map的键支持所有类型，而Object只支持String或Symbol
- 你可以通过size属性很容易地得到一个`Map的键值对个数，`而对象的键值对个数只能手动确认Object.keys()
- 使用的方式不一样。obj[key]或obj.key，map.set()和map.get(); object判断键使用in，map使用has
- Map中元素保持插入时的顺序，而Object按以下规则排序：
  - **非负整数**最先列出，按从小到大的顺序
  - 然后**字符串、负整数，浮点数**按插入顺序列出
  - 最后列出**Symbol**，其也按照插入顺序
- Map是可迭代对象可以使用for of或.foreach方法迭代；而Object只能通过for in获取键访问
...
**性能分析**https://blog.csdn.net/weixin_46837985/article/details/115475183
...
https://blog.csdn.net/weixin_42755677/article/details/92801922

拉链法实现的

![preview](D:\typora\work\src\v2-56a807e5ed10b5a2a025a7eacc58c3b2_r.jpg)
...
// map上的方法就是先找到key上的链，然后操作链表
```

**Map长链优化**

https://blog.csdn.net/QGhurt/article/details/109211485

参考Redis、Java，多半是变成B+树、红黑树或者使用多重映射，以及扩容table
...
https://blog.csdn.net/qq_41376740/article/details/80366143

使用一个队列存储弱引用，然后GC（垃圾回收）调WeakMap中存储在队列中的弱引用，同时清除队列。
...
遍历拷贝后的对象的，使用defineProperty劫持set和get
...
不会。使用ie浏览器可以查看options请求(chrome看不到)。可以看到options请求获得了响应，之后没有正式的请求。

![image-20210819141212403](E:\学习\review\src\image-20210819141212403.png)



### **JSONP的实现**

JSONP能实现跨域的核心在于<font style="background: #f1dcdc">script标签的src属性不被同源策略限制</font>

具体实现：

- 前端定义一个callback(data)函数，利用data进行处理
- 后端创建js文件，内容为callback(后台数据)
- 前端利用script标签引用这个js文件，这样就获取到了后端的数据(文件存数据)

更实际的应用，需要实现动态添加script标签的函数，后台实现get方法传入参数?callback=前端函数名，来返回一个由后端定义js文件名，内容为调用前端callback函数名文件的src。
...
用于非同源页面之间的跨域 **window.postMessage()**
...
https://blog.csdn.net/p3118601/article/details/88966864

#### **localStorage的页面通信**

监听storage事件，通过e.key判断来自那个同源页面。（localStorage是同源不同页面的通信）
...
`BroadcastChannel` 接口代理了一个命名频道，可以让指定 [origin](https://developer.mozilla.org/zh-CN/docs/Glossary/Origin) 下的任意 [browsing context](https://developer.mozilla.org/zh-CN/docs/Glossary/Browsing_context) 来订阅它。它允许同源的不同浏览器窗口，Tab页，frame或者 iframe 下的不同文档之间相互通信。通过触发一个 `message` 事件，消息可以广播到所有监听了该频道的 `BroadcastChannel` 对象。
...
https://www.cnblogs.com/bill-shooting/archive/2018/07/21/9347441.html

#### Shared Worker

[Shared Worker](https://link.juejin.im/?target=https%3A%2F%2Fdeveloper.mozilla.org%2Fen-US%2Fdocs%2FWeb%2FAPI%2FSharedWorker) 是 Worker 家族的另一个成员。普通的 Worker 之间是独立运行、数据互不相通；而多个 Tab 注册的 Shared Worker 则可以实现数据共享。
...
https://segmentfault.com/a/1190000015890245

![3793524098-5b665dbbde824_fix732](D:\typora\work\src\3793524098-5b665dbbde824_fix732.png)

父组件：beforeCreate初始化、created绑定数据，beforeMount加载render模板，（进入子组件生命周期）。

子组件：按父组件流程下去，直到mounted，子组件挂载完成父组件也随之挂载。

**销毁流程**

![image-20210917190555542](D:\typora\work\src\image-20210917190555542.png)

父组件：beforeDestroy实例销毁前，进入子组件销毁周期

子组件：触发beforeDestroy回调，destroyed完成销毁，父组件随之销毁



#### render函数

[Vue render函数 - 简书 (jianshu.com)](https://www.jianshu.com/p/7508d2a114d3)
...
不过template和el都要解析为AST（抽象语法树），然后再转为render函数去渲染。

每当Observer监听到数据的变化，通知Dep的Watcher进行视图更新。其中的视图更新就是使用render函数。

视图更新还能更细的分为生成Virtual DOM，进行Diff，通过Patch修改真实DOM。
...
​	如果在实例创建之后`添加新的属性`到实例上，它不会触发视图更新。使用Vue.set()方法可以让Vue监听到，并更新视图
...
**computed是对Vue API Watch的复杂的使用，即具有缓存功能的Watch**（这是原理性的，在后面讲）
...
<keep-alive> 包裹动态组件时，会缓存不活动的组件实例，而不是销毁它们。和 <transition> 相似，<keep-alive> 是一个抽象组件：它自身不会渲染一个 DOM 元素，也不会出现在父组件链中。 当组件在 <keep-alive> 内被切换，它的 `activated` 和 `deactivated` 这两个生命周期钩子函数将会被对应执行。 在 2.2.0 及其更高版本中，`activated` 和` deactivated` 将会在 <keep-alive> 树内的所有嵌套组件中触发。 主要用于保留组件状态或避免重新渲染。(运用了LRU算法/最近最少使用页面置换算法)
...
class HashRouter {
  ...
  hashChangeUrl() {
    ...
    */
    ...
    if (this.routes[this.currentHash] === undefined) throw new Error('404');
...
https://blog.csdn.net/qq_14993375/article/details/103981954

### **Vue源码部分**

飞跃疯人院vue源码系列真的香https://juejin.cn/post/6844903911669628941

#### **Vue2和Vue3双向绑定原理**

https://juejin.cn/post/6854573209673531400

https://juejin.cn/post/6844903911669628941

vue数据双向绑定是通过数据劫持结合发布者-订阅者模式的方式来实现的

​	**vue2**使用发布/订阅模式来做到数据与视图的绑定。为了能直接使用属性（不用Vue.set()）就更新视图，重写get和set方法，利用Object.defineProperty进行数据劫持。
...
​	![img](./src/1735ffad46ecf108_tplv-t2oaga2asx-watermark.awebp)

compile订阅数据变化就是依赖收集，即绑定好要使用数据的视图。不过依赖收集不但DOM，还有其他的变量，就是触发了get方法。
...
class Compile {
  constructor (el, vm) {
    ...
    this.vm = vm;
    // 1.获取文档碎片对象，放入内存，减少reflow和repaint
  ...
  compileElement(node) {
    ...
    [...attributes].forEach(attr => {
      ...
      // 判断当前name是否为指令 v-bind v-model v-on
  ...
  text(node, expr, vm) {
    ...
    if (expr.indexOf('{{') !== -1) {
      value = expr.replace(/\{\{(.+?)\}\}/g, (...args) => {
        // 绑定watcher从而更新视图
        // 去除首尾空格
        const target = args[1].replace(/^\s+|\s+$/g, '')
        new Watcher(vm, target, () => {
          this.updater.textUpdater(node, this.getContentVal(expr, vm));
        })
        return this.getVal(target, vm);
      })
    } else {
      // 也可能是 v-text='obj.name'  v-text='msg'
...
​	**vue3**使用Proxy替代Object.defineProperty。因为Proxy可以直接监听对象和数组的变化，并且有多达13种拦截方法。并且作为新标准将受到浏览器厂商重点持续的性能优化。
...
我们常使用的watch其实只是对Vue响应式原理中Watcher类的一种使用，简称为user-watcher。

（双向绑定中的wathcer称为render-watcher）
...
// 这是对this.$watch() API的封装
```

初始化时判断合并后的vm.$options是否有watch属性，有则进行watch初始化（initWatch）。
...
Vue.prototype.$watch = function(expOrFn, cb, options = {}) {
  ...
  const watcher = new Watcher(vm, expOrFn, cb, options)  // 实例化user-watcher。依赖收集
...
在监听属性的new Watcher和双向绑定有一些不同。

user-watcher会在constructor中设置deep、user和sync属性，以及对传入的expOrFn做判断为函数或是字符串模式。

当被监听的属性发生变化，触发update方法。
...
class Watcher {
  ...
  get() {
    ...
    if (this.deep) {  // 如果有定义deep属性
      traverse(value)  // 进行深度监听。递归触发数组或对象的get进行依赖收集。
    }
...
watch总结：为需要观察的数据创建并收集`user-watcher`，当数据改变时通知到`user-watcher`将新值和旧值传递给用户自己定义的回调函数。`sync`是不将`watcher`加入到`nextTick`队列而同步的更新、`immediate`是立即以得到的值执行一次回调函数、`deep`是递归的对它的子值进行依赖收集。

#### **Computed的原理**

computed也是Watcher类的一种实例，简称computed-watcher。它是`Watcher`类的最后也是最复杂的一种实例化的使用。

判断合并属性上是否存在computed属性，有则进行初始化。
...
function initComputed(vm, computed) {
  ...
  for(const key in computed) {
    ...
    if (!(key in vm)) {
      defineComputed(vm, key, getter) // 成为响应式的关键
    }
...
class Watcher{
  constructor(vm, expOrFn, cb, options) {
    ...
    this.value = undefined // 如果是其他的watcher就会调用get()方法
  }
...
重新定义get方法让computed成为响应式数据。

在执行`evaluate`方法进行求值的过程中又会触发`computed`内可以访问到的响应式数据的`get`，它们会将当前的`computed-watcher`作为依赖收集到自己的`dep`里，计算完毕之后将`dirty`置为`false`，表示已经计算过了。
...
computed只能监听响应式数据，不然无法做出响应。当依赖的数据改变时，dirty为true；然后视图/调用时进行依赖收集；调用Dep的notify方法去执行依赖收集到的watcher的update方法进行render。
...
https://zhuanlan.zhihu.com/p/116605265

`transition`组件是一个抽象组件，不会渲染出任何的`Dom`，它主要是帮助我们更加方便的写出动画。以插槽的形式对内部单一的子节点进行动画的管理，在渲染阶段就会往子节点的虚拟`Dom`上挂载一个`transition`属性，表示它的一个被`transition`组件包裹的节点，在`path`阶段就会执行`transition`组件内部钩子，钩子里分为`enter`和`leave`状态，在这个被包裹的子节点上使用`v-if`或`v-show`进行状态的切换。你可以使用`Css`也可以使用`JavaScript`钩子，使用`Css`方式时会在`enter/leave`状态内进行`class`类名的添加和删除，用户只需要写出对应类名的动画即可。如果使用`JavaScript`钩子，则也是按照顺序的执行指定的函数，而这些函数也是需要用户自己定义，组件只是控制这个的流程而已。
...
[Element UI Table组件源码分析 - SegmentFault 思否](https://segmentfault.com/a/1190000015756225)

[浅谈表格组件的实现：固定表头和固定列 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/33280304#:~:text=固定列. 固定表格头是通过把表格头和表格内容分成两个, 来实现，固定列的实现本质上是把固定列所需要的表格头和表格内容克隆一份来实现。实现原理如下图所示，固定列用的表头和表格体使用深色来表示：)

已经删减了许多。element的表格主要结构由column，header、body，summary，fixed组成。这几个部分都是独立在一个div中的，这是为了做到固定行（表头）、固定列的功能。当然这也带来的问题，行高与列宽的同步。

table、table-header、table-body、table-column之间通过table-store进行状态管理。table-header、table-body对table-store数据进行监听，每当table改变table-store数据时触发table-header、table-body通过render函数重新渲染。
...
2.store绑定到模板table-header，table-body
...
created() {
    ...
    // 设置列宽，判断是否应该更新高度去更新高度
      this.debouncedUpdateLayout = debounce(50, () => this.doLayout());
},
```

5.渲染header和body，通过调用render函数的方式

```js
mounted() {
      this.bindEvents();
      this.store.updateColumns(); // 更新列内容
      this.doLayout();  // 设置列宽，判断是否应该更新高度去更新高度
      ...
      this.$ready = true; // 用于判断列更新，高度和宽度
    // 调用 updateColumns，scheduleLayout（这个函数调用 debouncedUpdateLayout，updateColumns）
    }
```

**固定表头：**

将表头和body分开放置两个table里，设置div-table-body的高度。内容超过高度时，body滚动而不影响header。

注意设置header的gutter列，用于滚动条的占位，使header和body位置同步。

使用table-layout: fixed;布局。

**固定列：**

将固定的列拷贝一份表头和body，使用absolute布局。其中body也要单独放在div中，使其滚动不影响表头。
...
https://zhuanlan.zhihu.com/p/84837930

### **异常监控**

https://zhuanlan.zhihu.com/p/107853159
...
由于事件循环机制。同步任务完成才执行异步任务，如果同步任务时间过长，setTimeout将无法准确执行。
...
`JPEG 1992`标准有两种压缩算法：基于差值脉冲编码调制（DPCM）的`无损压缩` 和 基于离散余弦变换（DCT）的`有损压缩`

优点：

1. 对于视频而言，按帧压缩可以实现快速切入，有利于视频图像的编辑
2. 高压缩率，利于网络传输

缺点：

1. 有损压缩不利于高清图片、线条绘画，文字或图示（icon）的显示

`JPEG 2000`基于小波变换的图像压缩标准，压缩比更高，不会产生原先的基于离散余弦变换的JPEG标准产生的块状模糊瑕疵。JPEG 2000同时支持有损数据压缩和无损数据压缩；也支持更复杂的渐进式显示和下载。
...
是一种同时提供了有损压缩与无损压缩的图片文件格式，WebP支持无损压缩和透明色的功能，派生自视频编码格式VP8。

WebP 的优势体现在它具有更优的图像数据压缩算法，能带来更小的图片体积，而且拥有肉眼识别无差异的图像质量；同时具备了无损和有损的压缩模式、Alpha 透明以及动画的特性，在 JPEG 和 PNG 上的转化效果都相当优秀、稳定和统一。

优点：

1. 各方面都很优越（压缩算法、图像质量，透明支持，动画特性）
2. 与JPEG、PNG的转换效果相当优秀稳定
...
Worker 接口是 [Web Workers API ](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API)的一部分，指的是一种可由脚本创建的后台任务（不影响主线程），任务执行中可以向其创建者收发信息。
...
https://blog.csdn.net/aini423/article/details/105681488
...
但是和编译器类似，babel的转译过程也分为三个阶段：**parsing、transforming、generating**（解析、转义、生成）
...
https://www.cnblogs.com/sloong/p/5689162.html
...
// 这样写死了入口模块，很不方便。使用glob模块获取entry
```
...
**建议直接看美团这篇写得太好了**https://tech.meituan.com/2018/09/27/fe-security.html
...
​	`可能出现DOM漏洞，a标签的href属性执行scheme为JavaScript的语句`
...
- 可控性（Controllability）即出于国家和机构的利益和社会管理的需要，保证管理者能够对信息实施必要的控制管理，以对抗社会犯罪和外敌侵犯。
...
https://mp.weixin.qq.com/s/Q4MbHCUn_8MgWkdAKk0k9Q
...
所以简化下A.open(B)，A是B的父节点，window.open打开B，会自动保存A的信息，所以我们可以在B反向控制和修改A。
...
​	这个线程就是负责执行JS的主线程，前面说的"JS是单线程的"就是指的这个线程。大名鼎鼎的Chrome V8引擎就是在这个线程运行的。**需要注意的是，这个线程跟GUI线程是互斥的。互斥的原因是JS也可以操作DOM，如果JS线程和GUI线程同时操作DOM，结果就混乱了，不知道到底渲染哪个结果。这带来的后果就是如果JS长时间运行，GUI线程就不能执行，整个页面就感觉卡死了。**
...
​	定时器线程其实只是一个计时的作用，他并不会真正执行时间到了的回调，真正执行这个回调的还是JS主线程。所以当时间到了定时器线程会将这个回调事件给到`事件触发线程`，然后事件触发线程将它加到`事件队列`里面去。最终`JS主线程`从事件队列取出这个回调执行。事件触发线程不仅会将定时器事件放入任务队列，其他满足条件的事件也是他负责放进`任务队列`。
...
- 生命周期。在一定时间内失效，默认浏览器关闭失效。

**localStorage**

​	`localStorage`有一点跟`Cookie`一样，就是针对一个域名，即在**同一个域名下(同源)**，会存储相同的一段**localStorage**。
...
![img](D:\typora\work\src\1287779-20180404065845701-1111813120.png)
...
**宏任务**一般是：包括整体代码`script(AJAX、Event、整体代码)`，`setTimeout`，`setInterval`、`I/O`、`UI render`，	`setImmediate(Node.js)`。
**微任务**主要是：`Promise`、`Object.observe`、`MutationObserver`，`process.nextTick(Node.js)`。

![16693808-bb25042137a175ae](./src/16693808-bb25042137a175ae.webp)

- 每次执行宏任务前都检测是否存在微任务，存在则执行微任务队列，完成后再执行宏任务。

- 微任务队列执行完进行一次渲染；每完成一个宏任务进行一次渲染。
...
https://blog.csdn.net/xgangzai/article/details/106821553

https://segmentfault.com/a/1190000000440270

**栈中的垃圾回收**

函数运行结束销毁上下文。就是汇编里的栈平衡（栈顶指针恢复EBP寻址）。

**堆中的垃圾回收**

堆中的垃圾回收依赖于垃圾回收器。垃圾回收器只会针对新生代内存区、老生代指针区以及老生代数据区进行垃圾回收。

V8使用分代回收的策略，将内存分为两个生代： 新生代、老生代。

- 新生代存活时间较短的对象。64位机内存大小32MB，32位减半。
- 老生代中的对象为存活时间较长或常驻内存的对象。64位机1400MB，32位减半。

![img](D:\typora\work\src\format,png)

**新生代**

大多数的对象被分配在这里，这个区域很小但是垃圾回特别频繁。在新生代分配内存非常容易，我们只需要保存一个指向内存区的指针，不断根据新对象的大小进行递增即可。当该指针到达了新生代内存区的末尾（分半后），就会有一次清理（仅仅是清理新生代）

**新生代垃圾回收算法——副垃圾回收器**

新生代中采用 **Scavenge 算法** 处理，就是把新生代空间对半分为对象区域和空闲区域，新加入的对象会放到对象区域，当新生代（对象）区域快要被写满的时候就会执行一次垃圾清理的操作。

在对象区使用root搜索算法，从定义的root出发BFS遍历，将能遍历到的对象区的对象复制到空闲区。当对象指针和复制后的尾指针重合复制结束。此时空闲区的都是活跃对象，释放对象区的内存，然后将空闲区和对象区的身份互换。

**垃圾回收中的root**

通常包括但不限于以下几种，全局Window（每个iframe中），文档DOM树，存放在栈的变量，回收非活动对象所占据的内存，内存整理。
...
- 在一次从对象区到空闲区的复制时，检查它的内存地址来判断这个对象是否已经经历过一个新生代的清理。如果是代表其经历了两次清理，将它复制老生代中。
- 从对象区复制到空闲区时，如果空闲区已使用空间超过了整个新生代空间的25%，将整个对象复制到老生代。

**老生代**

老生代所保存的对象大多数是生存周期很长的甚至是常驻内存的对象，而且老生代占用的内存较多。

老生代还可划分为：老生指针区、老生数据区、大对象区、代码区、Call区、属性Cell区、Map区等等。

**老生代的垃圾回收算法——主垃圾回收器**

V8在老生代中的垃圾回收策略采用**Mark-Sweep（标记清除）**和**Mark-Compact（标记整理）**相结合。

V8使用了三色标记法进行标记。

- 黑色。已经被root引用到，且这个节点的相邻节点已经标机完。
- 灰色。已经被root引用到，但相邻节点还没有被处理。
- 白色。还没被访问到。
...
标机整理也是一样的，标记后将内存页上的对象向一个方向复制，使使用页是连续的，清除内存碎片。

**V8的优化**

JavaScript 是运行在主线程之上的，因此，一旦执行垃圾回收算法，需要将正在执行的 JavaScript 脚本暂停下来，待垃圾回收完毕之后再恢复脚本执行，我们把这个行为称之为 **全停顿(Stop-The-World)**。

为了解决全停顿带来的用户体验的问题，V8 团队进行多年的努力，向现有的垃圾回收器添加**并行、并发和增量**等垃圾回收技术，这些技术主要是从两个方面解决垃圾回收效率的问题：
...
由于垃圾回收机制的存在，开发者通常无需关心内存管理。但由于操作系统给浏览器分配的内存较小（放置内存消耗导致操作系统的崩溃），开发者还是需要优化内存占用。

优化内存占用的最佳手段就是保证在执行代码时只保存必要的数据。如果数据不再必要，那么把它设置为`null`，从而释放其引用。这也可以叫作**解除引用**。这个建议最适合全局变量和全局对象的属性。局部变量在超出作用域后会被自动解除引用。

不过要注意，解除对一个值的引用并不会自动导致相关内存被回收。解除引用的关键在于确保相关的值已经不在上下文里了，因此它在下次垃圾回收时会被回收。

除了解除引用，还可以使用其他手段进行内存优化

1. **使用const和let。块级作用域会让垃圾回收更早到来。**
2. **隐藏类和删除操作。**

每一个实例会关联一个隐藏类（参考new时候的原型指向，感觉隐藏类有点像原型）。运行期间，V8会将创建的对象与隐藏类关联起来，以跟踪它们的属性特征。能够共享相同隐藏类的对象性能会更好。
...
// 添加新属性，两个实例对应两个不同的隐藏类，产生性能影响
a2.author = "Jake";

```

对应以上的问题，**解决方案为“先创建再补充”式的动态属性赋值，并在构造函数中一次声明所以属性**：
...
// 删除某个实例的属性后，两个实例对应两个不同的隐藏类
delete a1.author;
```

解决此问题的最佳实践是，**将不要的属性手动设置为null**，这样可以保持隐藏类不变和继续共享，同时也能达到删除引用值供垃圾回收程序回收的效果

3. **静态分配与对象池**

这里的关键在于减少浏览器垃圾回收的次数。静态分配也包括数组，最好一开始就创建一个足够的大小。

```js
// 这个函数内部创建了块级变量，resultant对象的生命周期很短，如果频繁触发函数
// 垃圾回收程序可能也会频繁的调度
...
<font style="background-color: #fafafc">**注意**　静态分配是优化的一种极端形式。如果你的应用程序被垃圾回收严重地拖了后腿，可以利用它提升性能。但这种情况并不多见。大多数情况下，这都属于过早优化，因此不用考虑。</font>
...
![img](.\src\v2-0a7ef7d1a328dc37eadefb29e0ea705d_720w.jpg)

### **发布订阅模式**

和观察者模式类似，只不过中间多了个中间人（收集器），通过中间人去通知依赖对象。

![img](.\src\v2-b6ed65f370a766620718ad4227d5d4e5_720w.jpg)
...
OSI参考七层模型 和 TCP/IP参考模型

![img](D:\typora\work\src\705728-20160424234824085-667046040.png)

![img](D:\typora\work\src\705728-20160424234825491-384470376.png)
...
https://blog.csdn.net/weixin_38858749/article/details/109640666
...
会话层的功能和表示层的功能正好和SSL握手协议和记录协议的功能一样。
...
http://blog.itpub.net/31557835/viewspace-2219410/

![img](D:\typora\work\src\webp)

而在SSL中密钥交换算法有6种：无效（没有密钥交换）、RSA、匿名Diffie-Hellman、暂时Diffie-Hellman、固定Diffie-Hellman、Fortezza。

**HTTPS加密请求（四次握手）过程**

- 首先，客户端发起握手请求，以明文传输请求信息，包含版本信息，加密-套件候选列表，压缩算法候选列表，随机数，会话ID，扩展字段等信息。
...
- 服务器最后回应。（1）编码改变通知。表示随后的信息用协商好的算法和密钥进行。（2）服务器握手结束通知。表示握手结束，发送的字段是之前发送所有内容的hash值，供客户端校验。
- 传输加密后的信息，这部分信息就是服务端用私钥加密后的信息，可以在客户端用随机值解密还原。
- 客户端解密信息，客户端用之前生产的私钥解密服务端传过来的信息，于是获取了解密后的内容。整个过程第三方即使监听到了数据，也束手无策。

**sessionId和sessionTicket**

https://blog.csdn.net/firefile/article/details/80531978

Session ID 是服务端保存握手记录，Session Ticket 是 客户端保存握手记录。

通过Session ID得到Session Key直接完成SSL握手密钥交换，进入编码改变阶段。
...
![img](D:\typora\work\src\2012100614074631.png)

### **TLS**

[安全传输层协议](https://baike.baidu.com/item/安全传输层协议/17615457)（TLS）用于在两个通信[应用程序](https://baike.baidu.com/item/应用程序/5985445)之间提供保密性和[数据完整性](https://baike.baidu.com/item/数据完整性/110071)。（SSL是其前身）
...
https://www.cnblogs.com/feng9exe/p/8060349.html
...
**长连接（串行）**，同一个域名访问同一个文件的多个请求都可以复用一个tcp连接（不用像1.0一样 每次请求都需要重新建立连接）
依然存在的问题

- 多个请求只能被串行处理（数据基于文本，只能按顺序传输）
- 访问多个不同的文件依然会建立多个请求。

**多路复用（并行）：**同一个域名访问多个文件的请求也可以复用一个tcp连接，且多个请求可以被并行处理。
...
SSE的实现是依赖于流信息的存在（下载、视频流）。发送的不是一次性的数据包，而是一个数据流，会连续不断地发送过来。这时，客户端不会关闭连接，会一直等着服务器发过来的新的数据流。
...
二进制传输https://blog.csdn.net/weixin_34302561/article/details/88881259

WebSocket 是 HTML5 开始提供的一种在单个 `TCP 连接`上进行全双工通讯的协议（服务端推送功能）。
...
ws.binaryType = 'arraybuffer'; //WebSocket.binaryType 返回websocket连接所传输二进制数据的类型。
```
...
SSH为Secure Shell（安全外壳协议）的缩写，由IETF的网络工作小组（Network Working Group）所制定；SSH为建立在`应用层和传输层`基础上的安全协议。传统的网络服务程序，如FTP、.

SSH 主要由三部分组成：

- **传输层协议 [SSH-TRANS]**。提供了服务器认证，保密性及完整性。此外它有时还提供压缩功能。 SSH-TRANS 通常运行在[TCP/IP](https://baike.baidu.com/item/TCP%2FIP)连接上，也可能用于其它可靠数据流上。 SSH-TRANS 提供了强力的加密技术、密码主机认证及完整性保护。该协议中的认证基于主机，并且该协议不执行[用户认证](https://baike.baidu.com/item/用户认证)。更高层的用户认证协议可以设计为在此协议之上。

- **用户认证协议 [SSH-USERAUTH]**。用于向服务器提供客户端用户鉴别功能。它运行在[传输层](https://baike.baidu.com/item/传输层)协议 SSH-TRANS 上面。当SSH-USERAUTH 开始后，它从低层协议那里接收会话[标识符](https://baike.baidu.com/item/标识符)（从第一次[密钥](https://baike.baidu.com/item/密钥)交换中的交换哈希H ）。会话标识符唯一标识此会话并且适用于标记以证明私钥的所有权。 SSH-USERAUTH 也需要知道低层协议是否提供保密性保护。
...
​	允许死锁的出现。通过判断是否出现环，或使用等待超时法检测死锁。
...
一个好用的网站https://oi-wiki.org/graph/concept/
...
利用堆序性。将待排序元素全部入堆，然后全部弹出。插入和弹出时间复杂度O(logN)，进行n次总时间复杂度O(N*logN)
...
归并排序的思想也常常用来查询逆序对、小和计算

`逆序对`https://leetcode-cn.com/problems/shu-zu-zhong-de-ni-xu-dui-lcof/

`小和计算`

![屏幕截图 2021-09-23 201833](C:\Users\Administrator\Pictures\屏幕截图 2021-09-23 201833.png)

```js
// 计算总和
const a = [1, 3, 5, 2, 4, 6];
const s = merge(a, 0, a.length - 1);
console.log(s);

function merge(arr, L, R) {
    if (L >= R) return 0;
    const m = (L + R) >> 1;
    const t1 = merge(arr, L, m);
    const t2 = merge(arr, m + 1, R);

    let sum = 0;
    let i = L, j = m + 1, k = 0;
    const t = Array.from(arr);
    while (i <= m && j <= R) {
        if (arr[i] <= arr[j]) {
            sum += (R - j + 1) * arr[i]; // 代表后续小和中都有arr[i]
            t[k++] = arr[i++];
        } else {
            t[k++] = arr[j++];
        }
    ...
    return sum + t1 + t2; // 分组后的有序数组小和总和 + sum
}
```
...
将字符概率从小到大排序，取最小的两个组合为一颗子树，从列表删除两个字符概率，将概率和添加到列表（保证有序），继续之前的操作。

最后形成的二叉树就是哈夫曼树。

**m进制哈夫曼树**

https://zhidao.baidu.com/question/24937292.html

一般的哈夫曼树是二叉树（零一编码），但其实可以做为m叉数。

实现要点：

- 确定第一次选取的节点数，即第一个子树的分支数量。选取数量为节点数n % m。
- 后面都按概率排序选取m个。
...
堆是特殊的完全二叉树，可以利用 nL = 2\*n + 1, nR = 2\*n + 2的性质在数组上用下标实现父子节点的关系。

堆和完全二叉树的区别在于具有堆序性，满足父节点大于（小于）子节点，称为大根堆（小根堆）。

从树的深度推最多有logN层，则维护堆序性只需要O(logN)。

```js
class Heap {
  constructor(mode = 0, n = 0) {
    this.nodes = new Array(n);
    this.mode = mode; // 0: big; 1: small;
    this.length = 0;
  }
...
// 插入从后往前推，直到满足堆序性
  insert(val) {
    let cur = this.length; // 在最尾部插入
    let father = ((cur + 1) >> 1) - 1; // 推父节点下标
    const mode = this.mode;
    while (father >= 0) {
      // 不满足堆序性交换
      if ((mode === 0 && this.nodes[father] < val) ||
      (mode === 1 && this.nodes[father] > val)) {
        this.nodes[cur] = this.nodes[father];
      } else {
        // 满足堆序性退出
        break;
      }
  ...
  pop() {
    ...
    if (this.length) {
      ...
      this.length -= 1;
      const temp = this.nodes[this.length];
      let cur = 0;
      let left = 1, right = 2;
      // 以左子树为循环条件，右子树可能为空
      while (left < this.length) {
        // 满足堆序性退出。特判无右子树。
        if ((mode === 0 && temp > this.nodes[left] && (right === this.length || temp > this.nodes[right])) ||
        (mode === 1 && temp < this.nodes[left] && (right === this.length || temp < this.nodes[right]))) break;

        // 选择较大/较小的交换， 往子层继续判断。特判无右子树。
        if ((mode === 0 && this.nodes[left] > this.nodes[right]) ||
        (mode === 1 && this.nodes[left] < this.nodes[right]) ||
        right === this.length) {
          this.nodes[cur] = this.nodes[left];
          cur = left;
        } else {
          this.nodes[cur] = this.nodes[right];
          cur = right;
        }
        left = 2 * cur + 1;
        right = left + 1;
...
若 o的权值等于 v，该节点的附加域该值出现的次数自增 。

若 o的权值大于 v，在 o的左子树中插入权值为 v的节点。

若 o的权值小于 v，在 o的右子树中插入权值为 v的节点。

**删除**

定义 `del(o,v)` 为在以 为根节点的二叉搜索树中删除一个值为 的节点。

先在二叉搜索树中找到权值为 v的节点，分类讨论如下：

若该节点的附加 cnt大于 1，只需要减少 1。

若该节点的附加 cnt为 1：

若 o为叶子节点，直接删除该节点即可。

若 o为链节点，即只有一个儿子的节点，返回这个儿子。

若 o有两个非空子节点，一般是用它左子树的最大值或右子树的最小值代替它，然后将它删除。

#### **二叉平衡数AVL**

1. 空二叉树是一个 AVL 树
2. 如果 T 是一棵 AVL 树，那么其左右子树也是 AVL 树，并且 abs(h(ls) - h(rs)) <= 1，h 是其左右子树的高度
3. 树高为 O(logN)
...
![来自算法4](D:\typora\work\src\e3c98484881bd654daa8419bcb0791a2b6f8288b58ef50df70ddaeefc4084f48-file_1575215107950)

```js
function TrieNode() {
    let endNum = 0; // 为结束字符的次数
    const next = new Array(n); // 字符集，值为下一个node
}
```
...
dijkstra。单源最短路径算法。不能处理负权。

floyd。多源最短路径。

bellman-ford。单源再n-1条边下的最短路径。处理带边数限制的问题。
...
二分查找情况总结https://blog.csdn.net/yefengzhichen/article/details/52372407#commentBox

#### **快速选择**

时间复杂度O(n)，递归栈空间O(log n)

数组中第k大的元素https://leetcode-cn.com/problems/kth-largest-element-in-an-array/
...
var findKthLargest = function(nums, k) {
    ...
    function quick(arr, L, R) {
        ...
        let i = L;
        let j = R;
        const base = nums[i]; // base可以使用random随机化
        while (i !== j) {
            while (i < j && arr[j] <= base) j -= 1;
            if (i < j) arr[i++] = arr[j];
            while (i < j && arr[i] >= base) i += 1;
            if (i < j) arr[j--] = arr[i];
        }
        arr[i] = base;
        let res = false;
        if (k - 1 === i) return base;
        if (k - 1 < i) return quick(arr, L, i - 1);
        else return quick(arr, i + 1, R);
    }
};
```
...
动态规划往往可以由记忆化深度递归得出。——宫水三叶

解题步骤：**状态定义->递推公式->初始化**

**（动态规划想不出来时，不妨试试dfs，然后加个记忆化，最后得出dp）**
...
希望用一种规律搞定背包问题https://leetcode-cn.com/problems/combination-sum-iv/solution/xi-wang-yong-yi-chong-gui-lu-gao-ding-bei-bao-wen-/
...
4.如果是排列组合问题，是否需要考虑元素之间的顺序。需要考虑顺序有顺序的解法，不需要考虑顺序又有对应的解法。

**接下来讲一下背包问题的判定**
背包问题具备的特征：给定一个target，target可以是数字也可以是字符串，再给定一个数组nums，nums中装的可能是数字，也可能是字符串，问：能否使用nums中的元素做各种排列组合得到target。

**背包问题技巧：**
1.如果是0-1背包，即数组中的元素不可重复使用，nums放在外循环，target在内循环，且`内循环倒序`；
...
2.如果是完全背包，即数组中的元素可重复使用，nums放在外循环，target在内循环。且`内循环正序`。
...
3.如果排列问题需考虑元素之间的顺序，需将target放在外循环，将`nums放在内循环`。
...
`先手必胜`，石子总和为奇数，一定存在胜负，且只有偶数堆。

先手取石子可取首尾（奇数号或偶数号），而后手取的首尾只能取奇偶相同的石堆，所以先手先计算取奇数号和偶数号石堆的大小，就能获胜

#### **路径DP**

#### 数位dp

记忆化原始模板https://www.luogu.com.cn/blog/virus2017/shuweidp
...
var countDigitOne = function(n) {
    ...
    // 当前位，位数为1的计数，是否被限制
    function dfs(pos, sta, limit) { // sta代表满足的状态
        ...
        for (let i = 0; i <= up; i++) {
            ...
            // 若 ans += dfs(pos - 1, 1, ...)，结果会为 n + 1，即 0 ~ n个数字
        }
...
递推模板https://blog.csdn.net/hzf0701/article/details/116717851

例题https://leetcode-cn.com/problems/non-negative-integers-without-consecutive-ones/submissions/
...
var findIntegers = function(n) {
    ...
    const L = nums.length;
    const dp = new Array(L + 1); // [长度][最高位]
    for (let i = 0; i <= L; i++) {
        dp[i] = new Array(2).fill(0);
    }
    dp[1][0] = 1;
    dp[1][1] = 2;
    for (let i = 1; i <= L - 1; i++) { // 递推
        dp[i + 1][0] = dp[i][1]; // 右边分支
        dp[i + 1][1] = dp[i][0] + dp[i][1]; // 左边累加
    }
...
https://blog.csdn.net/qq_19446965/article/details/105334635

leetcode题https://leetcode-cn.com/problems/unique-binary-search-trees/
...
**h(n)= h(0)\*h(n-1) + h(1)\*h(n-2) + ... + h(n-1)h(0) (其中n>=2),这是n阶**递推关系

**还可以化简为1阶递推关系: 如h(n)=(4n-2)/(n+1)\*h(n-1)(n>1) h(0)=1**

![equation](D:\typora\work\src\equation.svg)
...
如果是判断是否能被 3 整除很好办，每一位加的和起来能否被 3 整除就行。因为10 % 3 = 1。

其他数需要判断base模上m的循环或恒等。如能否被22整除，除 10^0外，其他项循环10, 12, 10, 12...

一种方法是使用`进制转换`，再拿22整除为例。1826 = 22 * 83，将1825转为23进制得( 3a9 )

​	3 + 10 + 9 = 22，能整除22

还有一种方法是进行`因式分解`，因为是大整数，使用是在形成大整数时做因式分解

**判断某个点是否在三个点形成的三角形中**

target点和其他点两个点形成的角度和为360度，则其在里面；

小于360度则在外面。

![sanjiaoxing](D:\typora\work\src\sanjiaoxing.png)

**求平方根**

`二分法`：sqrt(x)一定在(0, x)之间，假设为 x / 2。令 t = x / 2，若 t^2 < x则，sqrt(x)在(t, x)之间；若 t^2 > x则，sqrt(x) 在(0, t)之间。一直不断逼近，直到目标精度。

`牛顿迭代法`：若存在f(x) 约等于 0 的近似值解，则 y = g(x) = x - f(x) / f′(x)中，y比x更精确。

如求30的平方根，f(x) = x^2 - 30，g(x) = x - (x^2 - 30) / (2*x)，令 x = 30 / 2 = 15，y = g(15) = 8.5,

在令 x = y 继续进行迭代得到要求的精度值
...
![image-20210927002453137](D:\typora\work\src\image-20210927002453137.png)
...
1. **一共4个学校，5名老师，要把5名老师分配到4所学校，每所学校至少一名老师，有几种分法**？

   将两个老师共同看做一个老师，有C(2, 5) = 10种绑定方法。

   绑定后进行组合，A(4, 4) = 24。总共有 10 * 24 = 240
...
1~5年老实工作，同时学习相关知识，向部门的leader学习，看看大佬们是怎么做的。

学习各种技术框架的优缺点，知道如何组织和管理项目让大家能以最快的速度达到项目要求，甚至要了解手下的技术特长来安排任务。
...
主要负责实现视觉和交互效果，以及与服务器通信，完成业务逻辑。它的核心价值在于实现用户体验，大型互联网公司的用户体验部门，一般会包括用户研究、交互设计、前端技术和视觉设计等方面的内容。

#### 你的优势是什么

对于应届毕业生中，项目经理丰富，有企业的实习经历，能更快适应工作。

#### **你的缺点是什么**

首先，我刚毕业，经验方面不足，我会在工作中积极完成工作，积累各方面经验。

口音较重。和别人说话时会尽量说慢些，声音大一些。

#### 遇到的最大的挑战是什么

大一的时候，什么都不懂参加了个，比赛。在压力下完成的作品。

大三春招时期找实习。

#### **你有什么兴趣爱好**

尽量能突出沟通能力（唱歌、运动），获得的奖项（刷算法题）
