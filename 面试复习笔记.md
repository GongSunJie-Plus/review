...
class Heap {
  constructor(mode = 0, n = 0) {
    this.nodes = new Array(n); // 静态空间性能会更优
    this.mode = mode;
    this.length = 0;
  }
 ...
 // 插入从后往前推，知道满足堆序性
  insert(val) {
    let cur = this.length; // 插入位置在最后
    let father = ((cur + 1) >> 1) - 1; // 从子节点推父节点
    while (father >= 0) { // 根节点下标 = 0
      if (this.nodes[father] < val) { // 不满足堆序性则交换值
 ...
 // 弹出第一个后，将最后一个提到第一个，从前往后推，直到满足堆序性
  pop() {
    if (this.length) {
      const res = this.nodes[0]; // 保存第一个的值
      this.length -= 1;
      this.nodes[0] = this.nodes[this.length]; // 最后一个提前
      let cur = 0;
      let left = 1, right = 2, temp = -1, nextCur = -1;
      while (left < this.length) { // 维护堆序性
        nextCur = cur;
        if (this.nodes[cur] < this.nodes[left]) {
          temp = this.nodes[cur];
          this.nodes[cur] = this.nodes[left];
          this.nodes[left] = temp;
          nextCur = left;
        }
        if (right < this.length && this.nodes[cur] < this.nodes[right]) {
          temp = this.nodes[cur];
          this.nodes[cur] = this.nodes[right];
          this.nodes[right] = temp;
          nextCur = right;
        }
        if (nextCur === cur) break; // 没有发送交换，则已经满足堆序性
        cur = nextCur;
        left = 2 * cur + 1;
        right = left + 1;
      }
