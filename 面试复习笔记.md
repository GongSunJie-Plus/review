...
![prototype5](D:\typora\work\src\prototype5.png)

![20180420233205727](D:\typora\work\src\20180420233205727.png)
...
`Promise.any()`跟`Promise.all()`方法很像，只有一点不同，就是不会因为某个 Promise 变成`rejected`状态而结束。只要一个是fulfilled就为fulfill
...
​	为了弥补HTTP在状态管理上的不足。浏览器会随请求自动携带domain、父domain，当前path。父path的cookie。
